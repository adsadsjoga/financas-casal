...
export default async function CarrosPage() {
  ...
  const [vehiclesRes, costsRes, installmentsRes, accountsRes] =
    await Promise.all([
      supabase
        .from("vehicles")
        .select("*")
  ...
  return (
    <CarrosClient
      ...
      installments={(installmentsRes.data ?? []) as VehicleInstallment[]}
      hasCashAccount={(accountsRes.data ?? []).some(
        (a) => a.type === "dinheiro" && a.name === "Dinheiro em mãos",
      )}
